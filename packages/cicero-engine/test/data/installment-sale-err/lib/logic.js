...
/**
 * Initialize the smart clause
 * @param {Context} context - the Accord context
 * @param {org.accordproject.installmentsale.InitializeRequest} context.request - the incoming request
 * @param {org.accordproject.installmentsale.InitializeResponse} context.response - the response
 * @AccordClauseLogicInit
 */
function initializeContract(context) {
    ...
    context.state =
        serializer.fromJSON({
            ...
            'total_paid' : 0.0,
            'next_payment_month' : req.firstMonth
        });
}

/**
 * Initialize the smart clause (Duplicate)
 * @param {Context} context - the Accord context
 * @param {org.accordproject.installmentsale.InitializeRequest} context.request - the incoming request
 * @param {org.accordproject.installmentsale.InitializeResponse} context.response - the response
 * @AccordClauseLogicInit
 */
function initializeContractDup(context) {
    var req = context.request;
    var contract = context.contract;

...
function orgXaccordprojectXinstallmentsaleXInstallmentSale_PayInstallment(context) {
  ...
  return new orgXaccordprojectXinstallmentsaleXInstallmentSale().PayInstallment(pcontext);
}
...
function orgXaccordprojectXinstallmentsaleXInstallmentSale_PayLastInstallment(context) {
  let pcontext = { 'request' : context.request, 'state': context.state, 'contract': context.contract, 'emit': context.emit, 'now': context.now};
  //logger.info('ergo context: '+JSON.stringify(pcontext))
  return new orgXaccordprojectXinstallmentsaleXInstallmentSale().PayLastInstallment(pcontext);
}
class orgXaccordprojectXinstallmentsaleXInstallmentSale {
  main(context) {
    ...
    var res1 = null;
    if (either(cast(["org.accordproject.installmentsale.ClosingPayment"],vX$match0))) {
      var vX$case0 = null;
      vX$case0 = toLeft(cast(["org.accordproject.installmentsale.ClosingPayment"],vX$match0));
      res1 = {"left" : {"$main": vX$case0}};
    } else {
      var vX$case1 = null;
      vX$case1 = toRight(cast(["org.accordproject.installmentsale.ClosingPayment"],vX$match0));
      res1 = {"right" : null};
    }
    var res8 = null;
    if (either(res1)) {
      ...
      if (!(equal(deref(unbrand(vrequest), "amount"), (deref(unbrand(vlstate_0), "balance_remaining") + deref(unbrand(vcontract), "DUE_AT_CLOSING"))))) {
        ...
        var vlstate = brand(["org.accordproject.installmentsale.InstallmentSaleState"],concat(concat(concat(concat({"stateId": "#1"}, {"status": "Fulfilled"}), {"balance_remaining": vbalance}), {"total_paid": vtotal_paid}), {"next_payment_month": 0}));
        var vlemit = bunion([brand(["org.accordproject.cicero.runtime.PaymentObligation"],concat(concat(concat(concat(concat({"contract": vcontract}, {"promisor": deref(unbrand(vcontract), "BUYER")}), {"promisee": deref(unbrand(vcontract), "SELLER")}), {"deadline": null}), {"amount": brand(["org.accordproject.money.MonetaryAmount"],concat({"doubleValue": deref(unbrand(vrequest), "amount")}, {"currencyCode": "USD"}))}), {"description": ((deref(unbrand(deref(unbrand(vcontract), "BUYER")), "partyId") + " should pay installment to ") + deref(unbrand(deref(unbrand(vcontract), "SELLER")), "partyId"))}))], vlemit_0);
        t2 = {"left" : concat(concat({"response": brand(["org.accordproject.installmentsale.Balance"],concat({"balance": vbalance}, {"total_paid": vtotal_paid}))}, {"state": vlstate}), {"emit": vlemit})};
      }
      res8 = t2;
    } else {
      var vX$case1_0 = null;
      ...
      var res3 = null;
      if (either(cast(["org.accordproject.installmentsale.Installment"],vX$match0))) {
        var vX$1 = null;
        vX$1 = toLeft(cast(["org.accordproject.installmentsale.Installment"],vX$match0));
        res3 = {"left" : {"$main": vX$1}};
      } else {
        var vX$2 = null;
        vX$2 = toRight(cast(["org.accordproject.installmentsale.Installment"],vX$match0));
        res3 = {"right" : null};
      }
      var res7 = null;
      if (either(res3)) {
        ...
        var vstate_0$0 = vlstate_1;
        var vemit_0$0 = vlemit_1;
        var vrequest_0$0 = vX$5;
        var vlstate_0$0 = vstate;
        ...
        if (!(((deref(unbrand(vcontract), "MIN_PAYMENT") <= deref(unbrand(vlstate_0$0), "balance_remaining")) && (compare(deref(unbrand(vlstate_0$0), "next_payment_month"),23) < 0)))) {
          ...
          if (!((deref(unbrand(vcontract), "MIN_PAYMENT") <= deref(unbrand(vrequest), "amount")))) {
            ...
            if (!((deref(unbrand(vrequest), "amount") <= deref(unbrand(vlstate_0$0), "balance_remaining")))) {
              ...
              var vtotal_paid$0 = (deref(unbrand(vlstate_0$0), "total_paid") + deref(unbrand(vrequest), "amount"));
              var vlstate$0 = brand(["org.accordproject.installmentsale.InstallmentSaleState"],concat(concat(concat(concat({"stateId": "#1"}, {"status": "WaitingForFirstDayOfNextMonth"}), {"balance_remaining": vbalance$0}), {"total_paid": vtotal_paid$0}), {"next_payment_month": natPlus(deref(unbrand(vlstate_0$0), "next_payment_month"), 1)}));
              var vlemit$0 = bunion([brand(["org.accordproject.cicero.runtime.PaymentObligation"],concat(concat(concat(concat(concat({"contract": vcontract}, {"promisor": deref(unbrand(vcontract), "BUYER")}), {"promisee": deref(unbrand(vcontract), "SELLER")}), {"deadline": null}), {"amount": brand(["org.accordproject.money.MonetaryAmount"],concat({"doubleValue": deref(unbrand(vrequest), "amount")}, {"currencyCode": "USD"}))}), {"description": ((deref(unbrand(deref(unbrand(vcontract), "BUYER")), "partyId") + " should pay installment to ") + deref(unbrand(deref(unbrand(vcontract), "SELLER")), "partyId"))}))], vlemit_0$0);
              t4 = {"left" : concat(concat({"response": brand(["org.accordproject.installmentsale.Balance"],concat({"balance": vbalance$0}, {"total_paid": vtotal_paid$0}))}, {"state": vlstate$0}), {"emit": vlemit$0})};
            }
            t5 = t4;
          }
          t6 = t5;
  ...
  PayLastInstallment(context) {
    ...
    if (!(equal(deref(unbrand(vrequest), "amount"), (deref(unbrand(vlstate_0), "balance_remaining") + deref(unbrand(vcontract), "DUE_AT_CLOSING"))))) {
      ...
      var vlstate = brand(["org.accordproject.installmentsale.InstallmentSaleState"],concat(concat(concat(concat({"stateId": "#1"}, {"status": "Fulfilled"}), {"balance_remaining": vbalance}), {"total_paid": vtotal_paid}), {"next_payment_month": 0}));
      var vlemit = bunion([brand(["org.accordproject.cicero.runtime.PaymentObligation"],concat(concat(concat(concat(concat({"contract": vcontract}, {"promisor": deref(unbrand(vcontract), "BUYER")}), {"promisee": deref(unbrand(vcontract), "SELLER")}), {"deadline": null}), {"amount": brand(["org.accordproject.money.MonetaryAmount"],concat({"doubleValue": deref(unbrand(vrequest), "amount")}, {"currencyCode": "USD"}))}), {"description": ((deref(unbrand(deref(unbrand(vcontract), "BUYER")), "partyId") + " should pay installment to ") + deref(unbrand(deref(unbrand(vcontract), "SELLER")), "partyId"))}))], vlemit_0);
      t1 = {"left" : concat(concat({"response": brand(["org.accordproject.installmentsale.Balance"],concat({"balance": vbalance}, {"total_paid": vtotal_paid}))}, {"state": vlstate}), {"emit": vlemit})};
    }
    return t1;
  }
  PayInstallment(context) {
    var vcontract = deref(context, "contract");
    var vrequest = deref(context, "request");
    var vemit = deref(context, "emit");
    var vstate = deref(context, "state");
    ...
    if (!(((deref(unbrand(vcontract), "MIN_PAYMENT") <= deref(unbrand(vlstate_0), "balance_remaining")) && (compare(deref(unbrand(vlstate_0), "next_payment_month"),23) < 0)))) {
      ...
      if (!((deref(unbrand(vcontract), "MIN_PAYMENT") <= deref(unbrand(vrequest), "amount")))) {
        ...
        if (!((deref(unbrand(vrequest), "amount") <= deref(unbrand(vlstate_0), "balance_remaining")))) {
          ...
          var vtotal_paid = (deref(unbrand(vlstate_0), "total_paid") + deref(unbrand(vrequest), "amount"));
          var vlstate = brand(["org.accordproject.installmentsale.InstallmentSaleState"],concat(concat(concat(concat({"stateId": "#1"}, {"status": "WaitingForFirstDayOfNextMonth"}), {"balance_remaining": vbalance}), {"total_paid": vtotal_paid}), {"next_payment_month": natPlus(deref(unbrand(vlstate_0), "next_payment_month"), 1)}));
          var vlemit = bunion([brand(["org.accordproject.cicero.runtime.PaymentObligation"],concat(concat(concat(concat(concat({"contract": vcontract}, {"promisor": deref(unbrand(vcontract), "BUYER")}), {"promisee": deref(unbrand(vcontract), "SELLER")}), {"deadline": null}), {"amount": brand(["org.accordproject.money.MonetaryAmount"],concat({"doubleValue": deref(unbrand(vrequest), "amount")}, {"currencyCode": "USD"}))}), {"description": ((deref(unbrand(deref(unbrand(vcontract), "BUYER")), "partyId") + " should pay installment to ") + deref(unbrand(deref(unbrand(vcontract), "SELLER")), "partyId"))}))], vlemit_0);
          t1 = {"left" : concat(concat({"response": brand(["org.accordproject.installmentsale.Balance"],concat({"balance": vbalance}, {"total_paid": vtotal_paid}))}, {"state": vlstate}), {"emit": vlemit})};
        }
        t2 = t1;
  ...
  init(context) {
    ...
    var vlstate_0 = vstate;
    var vlemit = vemit;
    var vlstate = brand(["org.accordproject.installmentsale.InstallmentSaleState"],concat(concat(concat(concat({"stateId": "#1"}, {"status": "WaitingForFirstDayOfNextMonth"}), {"balance_remaining": deref(unbrand(vcontract), "INITIAL_DUE")}), {"total_paid": 0}), {"next_payment_month": deref(unbrand(vcontract), "FIRST_MONTH")}));
    return {"left" : concat(concat({"response": brand(["org.accordproject.cicero.runtime.Response"],{})}, {"state": vlstate}), {"emit": vlemit})};
  }
}
...
function __dispatch(context) {
  let pcontext = { 'request' : context.request, 'state': context.state, 'contract': context.contract, 'emit': context.emit, 'now': context.now};
  //logger.info('ergo context: '+JSON.stringify(pcontext))
  return new orgXaccordprojectXinstallmentsaleXInstallmentSale().main(pcontext);
}
function __init(context) {
  ...
  //logger.info('ergo context: '+JSON.stringify(pcontext))
  return new orgXaccordprojectXinstallmentsaleXInstallmentSale().init(pcontext);
}

...
class Commands {
    ...
    /**
     * Create an archive from a template directory
     *
     * @param {string} templatePath to the template directory
     * @param {string} [archiveFile] the name of the archive file
     * @returns {object} Promise to the result of creating the archive
     */
    static async archive(templatePath, archiveFile) {
        const template = await Template.fromDirectory(templatePath);
        const archive = await template.toArchive();

        if (!archiveFile) {
            archiveFile = sanitize(template.getIdentifier(), {
                replacement: '_'
            }) + '.cta';
        }

        return fs_writeFile(archiveFile, archive);
    }
    ...
    /**
     * Set default params before we archive a template dir
     *
     * @param {object} argv the inbound argument values object
     * @returns {object} a modfied argument object
     */
    static validateArchiveArgs(argv) {
        ...
        if(argv.verbose) {
            logger.info(`archive a template in ${argv.template}`);
        }

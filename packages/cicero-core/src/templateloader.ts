...
import DefaultArchiveLoader from './loaders/defaultarchiveloader';
...
export default class TemplateLoader {
    ...
    static async fromArchive(Template, buffer, options) {
        ...
        const sampleFiles = await TemplateLoader.loadZipFilesContents(zip, SAMPLE_FILE_REGEXP);
        sampleFiles.forEach(async (sampleFile) => {
            const matches = sampleFile.name.match(SAMPLE_FILE_REGEXP);
            let locale = 'default';
            // Locale match found
     ...
     */
    static async fromDirectory(Template, path, options: any = {}) {
        ...
        const grammar = await TemplateLoader.loadFileContents(path, 'text/grammar.tem.md', false, false);
    ...
    static async loadZipFileBuffer(zip, path, required = false) {
        ...
        if (!zipFile && required) {
            ...
            return zipFile.async('nodebuffer').then(data => Buffer.from(data));
        }

...
class ParserManager {
    ...
    getParser() {
        if (!this.grammarAst) {
            throw new Error(
                'Must call setGrammar or buildGrammar before calling getParser'
            );
        }

        return new nearley.Parser(
            nearley.Grammar.fromCompiled(this.grammarAst)
        );
    }
    ...
    getTemplateAst() {
        if (!this.grammarAst) {
            throw new Error(
                'Must call setGrammar or buildGrammar before calling getTemplateAst'
            );
        }
    ...
    buildGrammar(templatizedGrammar) {
        ...
        const parser = new nearley.Parser(
            nearley.Grammar.fromCompiled(templateGrammar)
        );
    ...
    buildGrammarRules(ast, templateModel, prefix, parts) {
        ...
        ast.data.forEach((element, index) => {
            ...
            if (
                element &&
                (element.type !== 'Chunk' || element.value.length > 0)
            ) {
                Logger.debug(
                    'element '
                        .concat(prefix)
                        .concat(index, ' ')
                        .concat(JSON.stringify(element))
                );
        ...
        templateModel.getProperties().forEach((property, index) => {
            const sep =
                index < templateModel.getProperties().length - 1 ? ',' : '';
            const bindingIndex = this.findFirstBinding(
                property.getName(),
                ast.data
            );
    ...
    handleBinding(templateModel, parts, inputRule, element) {
        ...
        if (!action) {
            ...
            if (
                property.getType() === 'DateTime' ||
                element.type === 'FormattedBinding'
            ) {
                if (property.getType() !== 'DateTime') {
                    ParserManager._throwTemplateExceptionForElement(
                        'Formatted types are currently only supported for DateTime properties.',
                        element
                    );
                ...
                const format = element.format
                    ...
                    : '"MM/DD/YYYY"';
                const formatRule = DateTimeFormatParser.buildDateTimeFormatRule(
                    format
                );
                type = formatRule.name;
                const ruleExists = parts.modelRules.some(
                    rule => rule.prefix === formatRule.name
                );
                ...
                if (!ruleExists) {
                    parts.modelRules.push({
                        ...
                        symbols: [
                            ''
                                .concat(formatRule.tokens, ' ')
                                .concat(formatRule.action, ' # ')
                                .concat(propertyName, ' as ')
                                .concat(format)
                        ]
                    });
                }
            } else if (
                element.type === 'ClauseBinding' ||
                element.type === 'WithBinding'
            ) {
                const nestedTemplate = element.template;
                const nestedTemplateModel = this.template
                    .getIntrospector()
                    .getClassDeclaration(property.getFullyQualifiedTypeName());
                this.buildGrammarRules(
                    nestedTemplate,
                    nestedTemplateModel,
                    propertyName,
                ...
                element.type === 'UListBinding' ||
                element.type === 'OListBinding'
            ) {
                const separator =
                    element.type === 'UListBinding' ? '\n- ' : '\n1. ';
                const nestedTemplate = ParserManager.adjustListBlock(
                    element.template,
                    separator
                );
                const nestedTemplateModel = this.template
                ...
                type = element.fieldName.value;
                action = '\n{%\n  ([ '
                    .concat(propertyName + 'First', ', ')
                    .concat(propertyName, ' ]) => {\n    return [')
                    .concat(propertyName + 'First', '].concat(')
                    .concat(propertyName, ');\n}\n%}');
            } else {
    ...
    inferVariablesFromGrammar(namespace, name, type, templatizedGrammar) {
        ...
        this.variables = this.parseVariablesFromGrammar(
            this.grammarName,
            this.grammarType,
            this.templateAst.data
        );
    }
        ...
        branches.forEach(element => {
            if (!element.type.includes('Chunk')) {
                ...
                //If and IfElse Bindings are booleans
                    ...
                    element.type === 'IfBinding' ||
                    element.type === 'IfElseBinding'
                ) {
                ...
                //Clause Bindings - are treated more or less like objects.
                    ...
                    let newObj = this.parseVariablesFromGrammar(
                        clauseType,
                        'clause',
                    ...
                    if (
                        finishedObjects.some(
                            e =>
                                e.data.$class ===
                                this.namespace + '.' + clauseType
                        )
                    ) {
                        let index = finishedObjects.findIndex(
                            x =>
                                x.data.$class ===
                                this.namespace + '.' + clauseType
                        );
                        finishedObjects[index] = _.merge(
                            finishedObjects[index],
                            newObj
                        );
                    } else {
    ...
    generateDefaultDataForObj(object, concepts) {
        ...
        switch (object.type) {
            ...
            default:
                console.log(
                    'Building data for unrecognized type (not contract or string)'
                );
        }
        ...
        Object.keys(object.data).forEach(key => {
            if (key !== '$class') {
                if (typeof object.data[key] === 'string') {
                    ...
                    typeof object.data[key] === 'object' &&
                    typeof object.data[key] !== null
                ) {
                    let matchingObjects = concepts.filter(function(el) {
                    ...
                    console.log(
                        `ERROR - Unknown model structure when trying to parse CTO definitions from ${key}`
                    );
                }
            } else {

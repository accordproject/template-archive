 ...
 * @private
 */
export default class TemplateSaver {
    ...
    static async toArchive(template, language, options) {
        ...
        const packageFileContents = JSON.stringify(metadata.getPackageJson());
        ...
        if (metadata.getRequest()) {
            const requestFileContents = JSON.stringify(metadata.getRequest());
        ...
        scriptFiles.forEach(function (file) {
            const fileIdentifier = file.getIdentifier();
            const fileName = fileIdentifier.split(/[\\/]/).pop() || fileIdentifier;

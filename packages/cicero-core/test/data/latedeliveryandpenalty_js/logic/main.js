 ...
 */
function ioXclauseXlatedeliveryandpenaltyXLateDeliveryAndPenalty_latedeliveryandpenalty(context) {
  ...
  return new ioXclauseXlatedeliveryandpenaltyXLateDeliveryAndPenalty().latedeliveryandpenalty(pcontext);
}
class ioXclauseXlatedeliveryandpenaltyXLateDeliveryAndPenalty {
  main(context) {
    ...
    var res1 = null;
    if (either(cast(["io.clause.latedeliveryandpenalty.LateDeliveryAndPenaltyRequest"],vX$match0_1))) {
      var vX$case0 = null;
      vX$case0 = toLeft(cast(["io.clause.latedeliveryandpenalty.LateDeliveryAndPenaltyRequest"],vX$match0_1));
      res1 = {"left" : {"$main": vX$case0}};
    } else {
      var vX$case1 = null;
      vX$case1 = toRight(cast(["io.clause.latedeliveryandpenalty.LateDeliveryAndPenaltyRequest"],vX$match0_1));
      res1 = {"right" : null};
    }
    var res63 = null;
    if (either(res1)) {
      ...
      if (!(dateTimeIsBefore(vp1, vp2))) {
        ...
        if (!((!(deref(unbrand(vcontract), "forceMajeure")) || !(deref(unbrand(vrequest), "forceMajeure"))))) {
          t61 = {"left" : concat(concat({"response": brand(["io.clause.latedeliveryandpenalty.LateDeliveryAndPenaltyResponse"],concat({"penalty": 0.0}, {"buyerMayTerminate": true}))}, {"state": vlstate}), {"emit": vlemit})};
        } else {
          var vx = vnow;
          var vy = vagreed;
          var vz = "days";
          ...
          t61 = {"left" : concat(concat({"response": brand(["io.clause.latedeliveryandpenalty.LateDeliveryAndPenaltyResponse"],concat({"penalty": vcapped}, {"buyerMayTerminate": !((compare(vdiff,deref(unbrand(deref(unbrand(vcontract), "termination")), "amount")) <= 0))}))}, {"state": vlstate}), {"emit": vlemit})};
        }
        t62 = t61;
      }
      res63 = t62;
  ...
  latedeliveryandpenalty(context) {
    ...
    if (!(dateTimeIsBefore(vp1, vp2))) {
      ...
      var t60;
      if (!((!(deref(unbrand(vcontract), "forceMajeure")) || !(deref(unbrand(vrequest), "forceMajeure"))))) {
        t60 = {"left" : concat(concat({"response": brand(["io.clause.latedeliveryandpenalty.LateDeliveryAndPenaltyResponse"],concat({"penalty": 0.0}, {"buyerMayTerminate": true}))}, {"state": vlstate}), {"emit": vlemit})};
      } else {
        var vx = vnow;
        var vy = vagreed;
        var vz = "days";
        ...
        var vp0$2 = bunion([vpenalty], [((deref(unbrand(vcontract), "capPercentage") * deref(unbrand(vrequest), "goodsValue")) / 100.0)]);
        var vcapped = Math.min.apply(Math,vp0$2);
        t60 = {"left" : concat(concat({"response": brand(["io.clause.latedeliveryandpenalty.LateDeliveryAndPenaltyResponse"],concat({"penalty": vcapped}, {"buyerMayTerminate": !((compare(vdiff,deref(unbrand(deref(unbrand(vcontract), "termination")), "amount")) <= 0))}))}, {"state": vlstate}), {"emit": vlemit})};
      }
      t61 = t60;
    }
    return t61;
...
const contract = new ioXclauseXlatedeliveryandpenaltyXLateDeliveryAndPenalty();
function __dispatch(context) {
  let pcontext = { 'request' : context.request, 'state': context.state, 'contract': context.contract, 'emit': context.emit, 'now': context.now};
  //logger.info('ergo context: '+JSON.stringify(pcontext))
  return new ioXclauseXlatedeliveryandpenaltyXLateDeliveryAndPenalty().main(pcontext);
}
function __init(context) {
  ...
  //logger.info('ergo context: '+JSON.stringify(pcontext))
  return new ioXclauseXlatedeliveryandpenaltyXLateDeliveryAndPenalty().init(pcontext);
}

...
describe('DateTimeFormatParser', () => {
    ...
    describe('#buildFormatRules', () => {
        ...
        it('should parse D MMM YYYY HH:mm:ss.SSSZ', async function() {
            const result = (new DateTimeFormatParser()).buildFormatRules('"D MMM YYYY HH:mm:ss.SSSZ"');
            result[result.length-1].tokens.should.eql('D  " " MMM  " " YYYY  " " HH  ":" mm  ":" ss  "." SSS Z');
            result[result.length-1].action.should.eql('{% (d) => {return {"$class" : "ParsedDateTime",   "days": d[0],   "months": d[2],   "years": d[4],   "hours": d[6],   "minutes": d[8],   "seconds": d[10],   "milliseconds": d[12],   "timezone": d[13]};}%}');
        });

        it('should parse D MMM YYYY HH:mm:ss.SSS Z', async function() {
            const result = (new DateTimeFormatParser()).buildFormatRules('"D MMM YYYY HH:mm:ss.SSS Z"');
            result[result.length-1].tokens.should.eql('D  " " MMM  " " YYYY  " " HH  ":" mm  ":" ss  "." SSS Z');
            result[result.length-1].action.should.eql('{% (d) => {return {"$class" : "ParsedDateTime",   "days": d[0],   "months": d[2],   "years": d[4],   "hours": d[6],   "minutes": d[8],   "seconds": d[10],   "milliseconds": d[12],   "timezone": d[13]};}%}');
        });

        it('should parse DD MMM YYYY HH:mm:ss.SSS Z', async function() {
            const result = (new DateTimeFormatParser()).buildFormatRules('"DD MMM YYYY HH:mm:ss.SSS Z"');
            result[result.length-1].tokens.should.eql('DD  " " MMM  " " YYYY  " " HH  ":" mm  ":" ss  "." SSS Z');
            result[result.length-1].action.should.eql('{% (d) => {return {"$class" : "ParsedDateTime",   "days": d[0],   "months": d[2],   "years": d[4],   "hours": d[6],   "minutes": d[8],   "seconds": d[10],   "milliseconds": d[12],   "timezone": d[13]};}%}');
        });

        it('should parse D M YYYY', async function() {
            const result = (new DateTimeFormatParser()).buildFormatRules('"D M YYYY"');
            result[result.length-1].tokens.should.eql( 'D  " " M  " " YYYY ');
            result[result.length-1].action.should.eql('{% (d) => {return {"$class" : "ParsedDateTime",   "days": d[0],   "months": d[2],   "years": d[4]};}%}');
        });

        it('should parse DD/MM/YYYY', async function() {
            const result = (new DateTimeFormatParser()).buildFormatRules('"DD/MM/YYYY"');
            result[result.length-1].tokens.should.eql( 'DD  "/" MM  "/" YYYY ');
            result[result.length-1].action.should.eql('{% (d) => {return {"$class" : "ParsedDateTime",   "days": d[0],   "months": d[2],   "years": d[4]};}%}');
        });

        it('should parse YYYY-MM-DD', async function() {
            const result = (new DateTimeFormatParser()).buildFormatRules('"YYYY-MM-DD"');
            result[result.length-1].tokens.should.eql( 'YYYY  "-" MM  "-" DD ');
            result[result.length-1].action.should.eql('{% (d) => {return {"$class" : "ParsedDateTime",   "years": d[0],   "months": d[2],   "days": d[4]};}%}');
        });

        it('should parse MM/DD/YYYY', async function() {
            const result = (new DateTimeFormatParser()).buildFormatRules('"MM/DD/YYYY"');
            result[result.length-1].tokens.should.eql( 'MM  "/" DD  "/" YYYY ');
            result[result.length-1].action.should.eql('{% (d) => {return {"$class" : "ParsedDateTime",   "months": d[0],   "days": d[2],   "years": d[4]};}%}');
        });

        it('should parse MM/DD/YYYY HH:mm:ss Z', async function() {
            const result = (new DateTimeFormatParser()).buildFormatRules('"MM/DD/YYYY HH:mm:ss Z"');
            result[result.length-1].tokens.should.eql( 'MM  "/" DD  "/" YYYY  " " HH  ":" mm  ":" ss Z');
            result[result.length-1].action.should.eql('{% (d) => {return {"$class" : "ParsedDateTime",   "months": d[0],   "days": d[2],   "years": d[4],   "hours": d[6],   "minutes": d[8],   "seconds": d[10],   "timezone": d[11]};}%}');
        });

        it('should fail to parse MM/MMM/YYYY', async function() {
            (()=> (new DateTimeFormatParser()).buildFormatRules('"MM/MMM/YYYY"')).should.throw('Duplicate month');
        });

        it('should fail to parse D/DD/YYYY', async function() {
            (()=> (new DateTimeFormatParser()).buildFormatRules('"D/DD/YYYY"')).should.throw('Duplicate day');
        });
